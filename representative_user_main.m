...
rating_matrix = zeros(n_movie,n_user);

% 填入评分矩阵
for i=1:n_samples
    rating_matrix(data(i,2),data(i,1))=data(i,3);
end

%% 划分训练数据和测试数据
identity_movie = eye(n_movie);
s = RandStream('mt19937ar','Seed',0); % 设置seed,每次生成的打乱矩阵不变
perm_item = identity_movie(randperm(s,n_movie),:);
rating_matrix = perm_item*rating_matrix;

n_train = floor(0.8*n_movie);
